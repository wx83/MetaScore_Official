...
import linear_attention_transformer # need switch back 
...
def save_result(
    ...
    save_pianoroll(
        sample_dir / "png" / f"{filename}.png", music, (6, 4), preset="frame"
    )
    ...
    save_pianoroll(
        sample_dir / "png-trimmed" / f"{filename}.png", music, (30, 8)
    )
...
def main():
    ...
    logging.basicConfig(
        ...
        format="%(message)s",
        handlers=[
            logging.FileHandler(args.out_dir / "generate_four_cond.log", "w"),
    ...
    # while True:
        for i in tqdm.tqdm(range(args.n_samples), ncols=80):
            batch = next(data_iter)
            print(f"current batch is = {batch['name']}")
            # ------------
            # Ground truth
            # ------------
    ...
    #              # print(f"output shape = {tgt_start}")
    #             # Generate new samples
    #             generated = model.generate(
    #                 tgt_start,
    #                 args.seq_len,
