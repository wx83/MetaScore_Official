...
class MusicDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # print(f"filename = {filename}, data_dir = {data_dir}, tag_dir = {data_dir}")
        # print(f"filename = {encoding}, data_dir = {indexer}, tag_dir = {encode_fn}")
    # the filename should have no path at all
        ...
        # self.tag_dir = pathlib.Path(tag_dir) # Change to json file to load once
    ...
    def __getitem__(self, idx):
        ...
        # print(f"notes = {self.data_dir / name[2] / name[3]}")
        ...
        # text_path = self.tag_dir / name[2] / name[3]/f"{name}.txt"
        # should load preprocessed embeddding
        # text_path = self.tag_dir / name[2] / name[3]/f"{name}.npy"
        # print(f"tag ptah = {text_path}")
        # with open(tag_path, "r") as file:
        #     tag_dict = json.load(file) # open at representation to save memory
        # {"genre": [], "composer"[]:, "instrument":[], "complexity:[None]"}
        # print(f"tags is = {tag_list}")
        # text_path = self.tag_dir / name[2] / name[3]/f"{name}.json"
        seq = self.encode_fn(notes, self.encoding, self.indexer) # encode
        # Encode the notes
        # print(f"text = {type(text)} should be numpy array")
        # text = str(text[0]) # should be a string
        ## notes, encoding, indexer,tag_names,tag: tag can be a list, self.tags: name of tag
        if seq is None:
            print(f"empty sequence = {name}")
...
def main():
    ...
    # print(f"args.names = {args.names} {args.in_dir} {args.in_dir_tag}")
...
#             logging.info(f"Name: {batch['name'][0]}, seq = {batch['seq'][0][:15]}")
